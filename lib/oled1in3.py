...
Device_SPI = oledconfig.Device_SPI
Device_I2C = oledconfig.Device_I2C
...
class SH1106(object):
    def __init__(self):
        ...
        self._dc = oledconfig.DC_PIN
        self._rst = oledconfig.RST_PIN
        self._bl = oledconfig.BL_PIN
        self.Device = oledconfig.Device
    ...
    def command(self, cmd):
        if(self.Device == Device_SPI):
            ...
            oledconfig.spi_writebyte([cmd])
        else:
            oledconfig.i2c_writebyte(0x00, cmd)
    ...
    def ShowImage(self, pBuf):
        for page in range(0,8):
            ...
            for i in range(0,self.width):#for(int i=0;i<self.width; i++)
                if(self.Device == Device_SPI):
                    oledconfig.spi_writebyte([~pBuf[i + self.width * page]]);
                else :
                    oledconfig.i2c_writebyte(0x40, ~pBuf[i + self.width * page])
